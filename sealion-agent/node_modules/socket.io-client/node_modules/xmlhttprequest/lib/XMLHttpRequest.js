 ...
 * File changed for SeaLion to support proxy - Shubhansh <shubhansh.varshney@webyog.com>
...
var Url = require("url")
  ...
  , fs = require('fs')
  , SealionGlobal = require('../../../../../lib/global.js');
  ...
  this.send = function(data) {
    ...
    if (settings.async) {
      ...
      // Use the proper protocol
        //changes made for SeaLion to support proxy
        if(SealionGlobal.http_proxy && SealionGlobal.http_proxy.length) {
            doRequest = http.request;
            var proxy = Url.parse(SealionGlobal.http_proxy);

            options = {
                hostname : proxy.hostname,
                port : proxy.port ? proxy.port : 80,
                path : (ssl ? 'https://' : 'http://') + host + ':' + (port ? port :(ssl ? 443 : 80)) + uri,
                method : settings.method,
                headers : headers
            };
            if(typeof proxy.auth !== 'undefined') {
